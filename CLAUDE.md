# Speecher — instruções do projeto
...
O desenvolvimento alterna entre um **Windows** e um **macOS**, e o build de cada sistema só
...
- **Teto da reescrita voltou para 8s, sem retry (41839b1) — só no Mac por enquanto.** É
  ...
  v0.1.15 publicada NÃO a contém, e o Windows segue no teto de 4s. A avaliação é que lá está
...
- **Rebuild por cima do app instalado no macOS derruba as permissões, e os Ajustes mentem.**
  ...
  hash do executável: todo `tauri build` copiado para `/Applications/Speecher.app` invalida as
  duas, enquanto a lista dos Ajustes continua mostrando o Speecher marcado. O sintoma é o
  ...
  `tccutil reset ListenEvent com.leonardoverano.speecher` e o mesmo para `Accessibility`,
