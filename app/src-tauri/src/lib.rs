...
fn default_sidecar_paths() -> (String, String) {
    (
        r"C:\dev\speecher\.venv\Scripts\python.exe".into(),
        r"C:\dev\speecher\sidecar\stt_server.py".into(),
    )
}
...
fn default_sidecar_paths() -> (String, String) {
    ...
    (
        home.join("dev/speecher/.venv/bin/python").to_string_lossy().into_owned(),
        home.join("dev/speecher/sidecar/stt_server.py").to_string_lossy().into_owned(),
    )
}
...
fn config_dir() -> std::path::PathBuf {
    dirs::config_dir().unwrap().join("Speecher")
}
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            tray.menu(&menu)
                ...
                .tooltip("Speecher — segure o atalho para ditar")
        ...
        .expect("erro ao iniciar o Speecher")
        .run(move |_app, event| {
