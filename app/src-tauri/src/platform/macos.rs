...
// das próprias chaves. Decisão do projeto: app local, sem senha. O Windows segue
...
pub fn spawn_hotkey_listener(shared: Arc<Shared>, tx: Sender<Cmd>) {
    std::thread::spawn(move || {
        ...
        let tap = match CGEventTap::new(
            ...
            Err(()) => {
                eprintln!(
                    "[hotkey] CGEventTap falhou — conceda Monitoramento de Entrada ao Speecher \
...
pub fn spawn_sidecar(s: &Settings) -> Option<std::process::Child> {
    ...
    match std::process::Command::new(&s.python)
        .arg(&s.sidecar)
        .env("SPEECHER_STT_PORT", s.stt_port.to_string())
        .env("SPEECHER_PARENT_PID", std::process::id().to_string())
        .spawn()
    {
        Ok(c) => Some(c),
        Err(e) => {
